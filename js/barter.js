const BarterSystem = (function() {
    ...
    function updateListings() {
        ...
        listings.innerHTML = barterListings
            .map(listing => `
                <div class="barter-card">
                    <p>Has: ${listing.offering || listing.have}</p>
                    <p>Wants: ${listing.wanting || listing.want}</p>
                    <p>Category: ${listing.category}</p>
                    <p class="date">${listing.date ? listing.date.toLocaleDateString() : 'No date'}</p>
                </div>
            `).join('');
    }
...
// Update event listener to use the namespace
document.addEventListener('DOMContentLoaded', () => {
    BarterSystem.displayBarterListings();
});
